...
function postnew() {
    ...
    $.ajax({
    	// run locally
       // url : "http://localhost:7071/api/kurtwritemongo",
       ...
       // write to local copy of writeToPubSub
        ...
        url : "https://kurtwritetops.azurewebsites.net/api/kurtwritetops",
...
function createList(which, city) {
    ...
    // run in cloud
    $.get("https://kurtmongoreader.azurewebsites.net/api/kurtmongoreader/?name="+ param, function(data, status){
 

    
     // run local
    //  $.get("http://localhost:7071/api/kurtmongoreader/?name=" + param, function(data, status){ 
